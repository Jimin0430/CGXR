	...
	)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (depth < 0) continue;
            depth = fmaxf(depth, 0.01f);
            glm::vec3 scale = collected_scales[j];
            float max_scale = fmaxf(scale.x, fmaxf(scale.y, scale.z));

            float weight = expf(fminf(max_scale / depth, 20.0f)) + fminf(phi / (depth * depth), 1e6f) + phi*phi ;
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < CHANNELS; ch++)
			{
                float fg_contrib = w_fg_c[ch] > 0.0f ? C[ch] / w_fg_c[ch] * (1-T) : 0.0f;
                out_color[ch * H * W + pix_id] = fg_contrib + T * bg_color[ch];
	...
	)
{
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < CHANNELS; ch++)
		    {
		    float safe_w = fmaxf(w_fg_c[ch], 1e-6f);
		    out_color[ch * H * W + pix_id] = w_fg_c[ch] > 0.0f ? (C[ch] + bg_color[ch]) / safe_w : bg_color[ch];
            w_fg[ch * H * W + pix_id] = w_fg_c[ch];
