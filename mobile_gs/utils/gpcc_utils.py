...
def _tmc3_available(gpcc_codec_path: str) -> bool:
    import shutil
    return (os.path.exists(gpcc_codec_path)
            or os.path.exists(gpcc_codec_path + '.exe')
            or shutil.which('tmc3') is not None)
...
def compress_gpcc(x: torch.Tensor, gpcc_codec_path: str='./mpeg-pcc-tmc13/build/tmc3/tmc3') -> bytes:
    """
    Compress geometry point cloud by GPCC codec.
    Falls back to raw uint16 bytes when tmc3 is unavailable.
    """
    assert len(x.shape) == 2 and x.shape[1] == 3, f'Input data must be a 3D point cloud, but got {x.shape}.'

    if not _tmc3_available(gpcc_codec_path):
        data = x.cpu().numpy().astype(np.uint16)
        shape_bytes = np.array(x.shape[0], dtype=np.int64).tobytes()
        return _RAW_MAGIC + shape_bytes + data.tobytes()
...
def decompress_gpcc(strings: bytes, gpcc_codec_path: str='./mpeg-pcc-tmc13/build/tmc3/tmc3') -> torch.Tensor:
    """
    Decompress geometry point cloud by GPCC codec.
    Handles raw uint16 fallback format produced when tmc3 is unavailable.
    """
    if strings[:len(_RAW_MAGIC)] == _RAW_MAGIC:
        offset = len(_RAW_MAGIC)
        n = int(np.frombuffer(strings[offset:offset+8], dtype=np.int64)[0])
        data = np.frombuffer(strings[offset+8:], dtype=np.uint16).reshape(n, 3)
