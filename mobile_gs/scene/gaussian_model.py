...
class OpaictyPhiNN(nn.Module):
    ...
    def forward(self,  shs, scales, xyz, viewdirs, rotations ):
        ...
        # clamp before normalize to prevent inf/nan from overflow
        shs = torch.clamp(shs, -1e6, 1e6)
        scales = torch.clamp(scales, 0, 1e6)
...
class GaussianModel:
    ...
    def kmeans(self, param_data, code_list, index_list, svq_len, n_clusters, code_params):
        ...
        for i in range(param_data.shape[1]//svq_len):
            input_np = param_data[:, i*svq_len:(i+1)*svq_len].detach().cpu().float().numpy()
            if CUML_AVAILABLE:
                ...
                kmeans = MiniBatchKMeans(n_clusters=n_clusters, max_iter=100, n_init=1, batch_size=min(10000, len(input_np)))
                labels = kmeans.fit_predict(input_np)
                cluster_centers = kmeans.cluster_centers_
                codebook = torch.nn.Parameter(torch.from_numpy(cluster_centers).float().cuda())
                index = torch.from_numpy(labels).cuda().long()
